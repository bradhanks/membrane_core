defmodule Membrane.Pipeline do
  ...
  Refer to `Membrane.ChildrenSpec` for details on instantiating and linking children.
  ## Starting and supervision

  Start a pipeline with `start_link/2` or `start/2` functions. Because the pipeline always spawns under a dedicated supervisor,
  either function will return `{:ok, supervisor_pid, pipeline_pid}` in the case of success. The supervisor never restarts the pipeline,
  but it does ensure that the pipeline and its children terminate properly. If the pipeline needs to be restarted, it should be spawned under
  a different supervisor with the appropriate strategy.
  ...
  @typedoc """
  Defines options passed to the `start/3` or `start_link/3` functions and subsequently received
  ...
  @typedoc "List of configurations used by the `start/3` or `start_link/3` functions."
  @type config :: [config_entry()]

  @typedoc "Defines configuration value used by the `start/3` or `start_link/3` functions."
  @type config_entry :: {:name, name()}

  @typedoc """
  Defines return values of the `start/3` or `start_link/3` functions."
  """
  @type on_start ::
  ...
  @doc """
  Starts the pipeline based on the given module and links it to the current
  process.
  ...
  * `asynchronous?` - if set to `true`, pipeline termination won't be blocking and
    will be executed in the process whose pid is returned as a function result. If
  ...
  * `force?` - determines how to handle a pipeline still alive after `timeout`.
    ...
    `{:error, :timeout}`. If set to `false`, it raises an error. Defaults to `false`.
  ...
  * `{:error, :timeout}` - pipeline was killed after `timeout`.
    ...
    after
      timeout ->
        if force? do
          ...
          raise PipelineError, """
          Pipeline #{inspect(pipeline)} hasn't terminated within the given timeout (#{inspect(timeout)} ms).
  ...
  @doc """
  Checks whether a module is a pipeline.
  """
